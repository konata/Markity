...
chrome.tabs.onUpdated.addListener((tab, change, page) => {
...
async function open(href: string, tab: number | undefined) {
  ...
  try {
    await chrome.tabs.update(tab, { url: href });
    return { ok: true };
  } catch (error) {
    return { ok: false, error: error instanceof Error ? error.message : String(error) };
  }
...
async function dispatch(action: string, payload: Record<string, unknown> = {}) {
  ...
  try {
    return await chrome.tabs.sendMessage(page.id, { ...payload, target: "markity", action });
  } catch (error) {
    return { ok: false, error: error instanceof Error ? error.message : String(error) };
  }
}
